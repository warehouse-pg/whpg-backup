...
cat >> $CLUSTER_CONFIG <<-EOF
	...
	# WHPG6's gpinitsystem (GPDB6-vintage) still looks for the pre-"coordinator"
	# renaming variable name and fatals with "MASTER_HOSTNAME variable not set"
	# without it; WHPG7/WHPG19 only read COORDINATOR_HOSTNAME above and ignore
	# this one. Setting both keeps this one vendored demo_cluster.sh working
	# across all three lanes, per its own "version-tolerant" design (README.md).
...
# Disable parallel query workers — PG18 parallel query is not yet
# compatible with GP's Motion/interconnect infrastructure and hangs.
# GPDB6 (PG9.4-based) predates parallel query entirely and doesn't recognize
# this GUC at all -- unlike an unknown GUC warning, an unrecognized name in
# postgresql.conf is a hard "contains errors" FATAL at startup, so this must
# be skipped there rather than just being a silent no-op.
if ! "${GPHOME}/bin/postgres" --gp-version 2>/dev/null | grep -q '(Greenplum Database) 6\.'; then
    echo "max_parallel_workers_per_gather=0" >> ${CLUSTER_CONFIG_POSTGRES_ADDONS}
fi

# Lower snapshot wait timeout from default 2h to 30s — prevents long
# hangs when dispatch timing causes Reader to wait for Writer snapshot.
echo "gp_interconnect_setup_timeout=30" >> ${CLUSTER_CONFIG_POSTGRES_ADDONS}
