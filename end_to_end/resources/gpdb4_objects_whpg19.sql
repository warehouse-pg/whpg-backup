...
-- PG19 dropped support for postfix operators ("Postfix operators are not
-- supported" / "operator right argument type must be specified"), so this
-- operator is defined as a prefix operator (RIGHTARG, built-in `factorial`)
-- instead of postfix (LEFTARG, `numeric_fac`) -- same pattern already used
-- in integration/predata_operators_queries_test.go for WHPG19.
CREATE OPERATOR #### (
    PROCEDURE = factorial,
    RIGHTARG = bigint
);
...
-- PG19 dropped the legacy RECHECK keyword on operator class OPERATOR entries
-- (syntax error, not just a no-op as in older GPDB/Postgres).
...
-- PG19 removed the MULE_INTERNAL encoding; use a conversion pair that still
-- exists there instead (same substitution already used in
-- integration/predata_functions_create_test.go's PrintCreateConversionStatements
-- test for WHPG19).
CREATE CONVERSION testconv FOR 'LATIN1' TO 'UTF8' FROM iso8859_1_to_utf8;
