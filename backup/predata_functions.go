...
func PrintCreateTransformStatement(metadataFile *utils.FileWithByteCount, objToc *toc.TOC, transform Transform, funcInfoMap map[uint32]FunctionInfo, transformMetadata ObjectMetadata) {
	...
	if fromSQLIsDefined {
		...
		gplog.Warn("No FROM function found for transform object with type %s and language %s", TypeFQN, transform.LanguageName)
	}
	...
	if toSQLIsDefined {
		...
		gplog.Warn("No TO function found for transform object with type %s and language %s", TypeFQN, transform.LanguageName)
	}
	statement += ");"
